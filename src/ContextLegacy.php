...
/**
 * @file
 * Provides the Aegir\Provision\ContextLegacy class.
 *
 * This is an exact copy of class Provision_Context. Much of this can likely be
 *   removed as we simplify everything.
 */
...
class ContextLegacy
{
    ...
    /**
     * Retrieve value from $properties array if property does not exist in
     * class
     * proper. Properties that refer to Provision_Context objects will be run
     * through d(), see is_oid().
     *
     * TODO: consider returning a reference to the value, so we can do things
     * like:
     *       `$this->options['option'] = 'value'` and it will correctly set it
     * in the drush context cache.
     */
    function __get($name)
    {
        if ($name == 'options') {
            return array_merge(
              provision_sitealias_get_record($this->name),
              array_filter(drush_get_context('stdin')),
              array_filter(drush_get_context('options')),
              array_filter(drush_get_context('cli'))
            );
        }
    ...
    /**
     * Store value in properties array if the property does not exist in class
     * proper.
     */
    ...
    /**
     * Check the properties array if the property does not exist in the class
     * proper.
     */
    function __isset($name)
    {
        return $name === 'options' || isset($this->properties[$name]) || property_exists(
            $this,
            $name
          );
    }
    ...
    function init()
    {
        ...
        foreach ($service_list as $service => $default) {
            ...
            if (method_exists($class, $func)) {
                call_user_func([$class, $func], $this);
            }
        }
    ...
    {
        $config = new Provision_Config_Drushrc_Alias(
          $this->name,
          $this->properties
        );
        $config->write();
     ...
     * been registered with service_subscribe, otherwise it will return the
     * value specified by the property specified by $this->parent_key.
     *
     * @param $service
    ...
    function service($service, $name = null)
    {
        if (isset($this->service_subs[$service])) {
            return d($this->service_subs[$service])->service(
              $service,
              ($name) ? $name : $this->name
            );
        } elseif (!is_null($this->parent_key)) {
            return $this->{$this->parent_key}->service(
              $service,
              ($name) ? $name : $this->name
            );
        } else {
    ...
    function services_invoke($callback, $args = [])
    {
        ...
        foreach (array_keys($services) as $service) {
            $results[$service] = provision::method_invoke(
    ...
    /**
     * Return the merged list of services and the associated servers supplying
     * them.
     *
     * This function will check with the parent_key to retrieve any service
     * subscription it may have, and will add any additional subscriptions.
     *
     * Once the call chain reaches the @server_master object, it will provide
     * the fallbacks if no subscriptions were available.
     */
