...
location = /<?php print $subdir; ?>/robots.txt {
  ...
  log_not_found off;
<?php if ($nginx_config_mode == 'extended'): ?>
  try_files /<?php print $subdir; ?>/sites/$server_name/files/robots.txt $uri @cache;
<?php else: ?>
  try_files /<?php print $subdir; ?>/sites/$server_name/files/robots.txt $uri @drupal;
<?php endif; ?>
}
...
<?php else: ?>
location ~* ^/<?php print $subdir; ?>/(index|cron|boost_stats|update|authorize|xmlrpc)\.php$ {
  add_header   X-Engine "Aegir";
<?php endif; ?>
  set $real_fastcgi_script_name $1.php;
  tcp_nopush   off;
